...
'''
Renaming the input images for LASCAR
...
if __name__ == '__main__':
    ...
    imagesTs_folder_LA_Scar_Cavity = nnunet_folder + '/nnUNet_raw_data/' + 'Task402_LA_Scar_Cavity/' + "imagesTs/"


    # if not osp.exists(imagesTs_folder_LA_Cavity):
    #     os.makedirs(imagesTs_folder_LA_Cavity)
    if not osp.exists(imagesTs_folder_LA_Scar_Cavity):
        os.makedirs(imagesTs_folder_LA_Scar_Cavity)
    ...
    for case in os.listdir(input_folder):
        ...
        inputcase_nnunet = os.path.join(imagesTs_folder_LA_Scar_Cavity, case.replace(".nii.gz", "_0000.nii.gz") )
